...
export async function Curriculo() {
  const { curriculo } = makePageGateway()
  const result = await curriculo.getCurriculo()

  const downloadUrl = result?.props?.url
      ...
      <div className="flex sm:flex-row border-b-2 sm:border-b-3 md:border-b-5 border-[#5A5F51] w-full justify-between items-center py-5 sm:py-10 md:py-15">
        ...
        {downloadUrl && (
          <a
            href={downloadUrl}
            download
            className="bg-olive p-2 sm:p-3 rounded-full hover:scale-110 transition cursor-pointer"
          >
            <DownloadSimple
              size={20}
